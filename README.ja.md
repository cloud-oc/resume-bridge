...
`Resume Bridge` は、候補者が準備した履歴書・職務経歴の情報を、分散し形式がばらばらなオンライン応募フォームへ橋渡しする、という意味です。自動送信を目的としたものではありません。資料整理、項目検出、入力補助、送信前レビューを支援します。

GitHub リポジトリ名とパッケージ名：`resume-bridge`。
...
| 3 段階マッチング | ルールマッチング、意味的な曖昧マッチング、任意の LLM 補完 |
...
| 多言語 UI | 拡張機能とヘルプ内容は英語、中国語、日本語に対応 |
| Firefox パッケージ | Chromium と Firefox WebExtension の両方を生成 |
...
3. 任意：履歴書インポート、自由回答、AI 補完が必要な場合は AI モデルを設定します。
